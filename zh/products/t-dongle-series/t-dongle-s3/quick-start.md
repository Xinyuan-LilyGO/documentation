...
T-Dongle-S3 搭载 0.96 英寸 **ST7735 IPS TFT**（80 × 160），由 **TFT_eSPI** 驱动。LVGL 刷新回调使用 `pushColors`。
...
打开仓库中的 `LVGL_Demo` 或 `Factory` 示例，这是 T-Dongle-S3 生产级 LVGL 集成的权威参考。
...
**Q：上传后屏幕不亮？**  
A：确认在调用 `tft.init()` 前已将 `TFT_BL`（GPIO38）设为 HIGH。

**Q：APA102 LED 不亮？**  
A：确认引脚顺序正确（CI=GPIO40，DI=GPIO39）。APA102 使用类 SPI 协议，时钟和数据为独立引脚，与 WS2812 单线协议不同。
